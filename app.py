...
def _render_bottom_expanders():
    ...
    with st.expander("このPoCで行わないこと"):
        st.markdown("**A. 職員・専門職が判断すること**")
        st.caption(
            "本PoCは、計画書と記録から確認材料を整理するものです。"
            "本人の状態・希望・ケアプランを踏まえた最終判断は、職員・専門職が行います。"
        )
        st.markdown(
            """
            - 医療上の診断や判断
        ...
        st.markdown("**B. 現在のPoCでは未実装の機能**")
        st.caption("これらは将来的な実運用を想定した機能であり、現在のPoCでは実装していません。")
        st.markdown(
            """
            - 特定介護ソフトとの自動連携
            - データベースへの永続保存
            ...
            - 監査ログ
            - 業務チャットへの通知
            - 実利用者データを用いた運用検証（公開デモでは実利用者データを使用していません。）
            """
        )
    with st.expander("将来構想"):
        st.markdown(
            """
            現在は未実装ですが、以下のような発展を構想しています。

            - 介護ソフトからのCSV取込形式の拡張
            - 外部AIによる文章表現の補助
            - 職員確認済み内容の多言語表示（やさしい日本語、英語、ベトナム語、インドネシア語等への翻訳）
            - 介護用語の正確性確認
            - 翻訳後も職員確認を必須とする設計
            - データベース、認証、権限管理、監査ログ
            - 利用者、訪問予定、ケアプラン、直近記録に応じた確認項目を介護記録画面内に表示し、観察と記録作成を支援する機能（現在は未実装）

            正式な翻訳や職員研修を代替するものではありません。
            """
        )
